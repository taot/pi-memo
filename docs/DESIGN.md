...
每个工具的 `promptGuidelines` 都要有一条**触发条件**——说清在什么可观察的时刻去够这个工具，
原有的分类法和限制作为从句保留。只有分类法（“写成哪种”）而没有触发条件（“何时写”）时，
模型全程不会调用；这是 `eval/agent-smoke/NOTES.md` 结论 1 实测出来的。

- `memory_recall`：上手不熟悉的部分、或撞到没见过的报错和约定时，先搜一次再从零规划；
  索引只有标题且有条数上限。索引显示没有记忆时跳过，避免冷启动空转。
- `memory_recall`：索引里有相关条目时先取全文，不根据标题猜测内容。
- `memory_write`：可核实的原因或事实写成 `env`；策略和“下次怎么办”写成 `exp`，必要时分别写两条。
- `memory_write`：收尾前判断这次学到的东西是否“早知道能省时间”。显而易见的（代码、题面或
  一条命令就能看出的）和过程流水账不写；没有达标的就不写。**什么算重要交给模型判断，不列清单。**
- `memory_revise`：召回的记忆与刚观察到的事实不符时立刻改，不放着不管，也不另写一条。
- `memory_forget`：记忆是错的、或指向仓库里已经不存在的文件/命令/行为时才删；这次没用上的留着。
